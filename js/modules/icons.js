import grunticon from 'grunt-grunticon';
...
class Icons {
  ...
  initSocialIconAnimation() {
    ...
    this.socialIcons.forEach((socialIcon) => {
      document.querySelectorAll('.icon-' + socialIcon.name).forEach((icon) => {
        const s = Snap(icon);
        s.hover(() => {
  ...
  doMenuAnimation(toggled) {
    ...
    const s = Snap('#nav-lines');
    const g = s.select('g');
    g = toggled ? g.animate({ stroke: '#F2F2F2' }, 300, mina.easeinout ) : g.animate({ stroke: '#243040' }, 300, mina.easeinout );

    lines.forEach((line) => {
      const l = s.select(line.el);
      const state = !toggled ? line.animProperties.from : line.animProperties.to;
